...
//! return-typed wrappers) live in version-specific submodules ([`super::v30`]).
...
///
/// Set via the version Cargo features at compile time (only `30_0` exists today; more are added
/// as their bindings are generated). Used in error messages and the version-mismatch heuristic.
...
const fn configured_version() -> &'static str {
    ...
    #[cfg(not(feature = "30_0"))]
    {
        compile_error!(
            "the `client-async` feature requires a version feature (`30_0`) to be enabled"
        )
    }
}

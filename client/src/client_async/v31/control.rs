...
/// Optional parameters for the `getopenrpcinfo` JSON-RPC method (consumed by `Client::get_open_rpc_info_with`).
#[derive(Clone, Debug, Default, serde::Serialize)]
#[serde(rename_all = "camelCase")]
pub struct GetOpenRpcInfoOptions {
...
impl Client {
    ...
    /// `getopenrpcinfo` with all optional arguments via [`GetOpenRpcInfoOptions`].
    ///
    /// Returns an OpenRPC document for currently available RPC commands.
    pub async fn get_open_rpc_info_with(
        &self,
        opts: GetOpenRpcInfoOptions,
    ) -> Result<GetOpenRpcInfo> {
        self.call_raw("getopenrpcinfo", &[json!(opts.show_hidden)]).await
    }
